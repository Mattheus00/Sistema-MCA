...
describe("envioBoletosUtils", () => {
  ...
  it("identifica itens que bloqueiam envio apenas pelo status da API", () => {
    expect(
      itemBloqueiaEnvio(makeItem({ status: "AGUARDANDO_CORRECAO", emailDestinatario: "a@b.com" })),
    ).toBe(true);

    expect(
      itemBloqueiaEnvio(makeItem({ status: "PRONTO_PARA_ENVIO", emailDestinatario: "" })),
    ).toBe(false);

    expect(
      itemProntoParaEnvio(makeItem({ status: "PRONTO_PARA_ENVIO", emailDestinatario: "" })),
    ).toBe(true);
    ...
    expect(
      itemBloqueiaEnvio(
        makeItem({ metodoIdentificacao: "NAO_IDENTIFICADO", status: "NAO_IDENTIFICADO" }),
      ),
    ).toBe(true);
  });
  ...
  it("retorna indicador visual exclusivamente pelo status da API", () => {
    expect(
      indicadorStatusItem(
        makeItem({ status: "ENVIADO", emailDestinatario: "a@b.com", clienteNome: "A" }),
      ).texto,
    ).toBe("Enviado");

    expect(
      indicadorStatusItem(
        makeItem({ status: "ERRO", emailDestinatario: "a@b.com", clienteNome: "A" }),
      ).cor,
    ).toBe("vermelho");

    expect(
      indicadorStatusItem(
        makeItem({ status: "IGNORADO", emailDestinatario: "a@b.com", clienteNome: "A" }),
      ).cor,
    ).toBe("cinza");
    ...
    expect(
      indicadorStatusItem(
        makeItem({ status: "PRONTO_PARA_ENVIO", clienteNome: "A", emailDestinatario: "" }),
      ).texto,
    ).toBe("Pronto para envio");

    expect(
      indicadorStatusItem(
        makeItem({ status: "AGUARDANDO_CORRECAO", clienteNome: "A", emailDestinatario: "" }),
      ).texto,
    ).toBe("Aguardando correção");
  });

  it("exibe sem e-mail apenas para AGUARDANDO_CORRECAO sem destinatário", () => {
    expect(
      exibirEmailItem(makeItem({ status: "AGUARDANDO_CORRECAO", emailDestinatario: "" })).texto,
    ).toBe("Sem e-mail");

    expect(
      exibirEmailItem(makeItem({ status: "PRONTO_PARA_ENVIO", emailDestinatario: "" })).texto,
    ).toBe("E-mail não encontrado");
  });
  ...
  it("identifica itens que precisam de confirmação", () => {
    ...
    expect(
      itemPrecisaConfirmacao(
        makeItem({ status: "AGUARDANDO_CORRECAO", confiancaIdentificacao: "ALTA" }),
      ),
    ).toBe(true);

    expect(
      itemPrecisaConfirmacao(
        makeItem({ status: "PRONTO_PARA_ENVIO", confiancaIdentificacao: "ALTA" }),
      ),
    ).toBe(false);
  });
});
